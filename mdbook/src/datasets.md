...
| ---- | :----: | :----: | :----: |
| Artix-7 | \\( 2 ^ {24} \\) | \\( 2 ^ {24} \\) | \\( 2 ^ {24} \\) |


## Files organization and dataset reading

The datasets are available for download at TODO. They are organised per
target, according to the file tree represented below. In particular, the 
datasets measured for the Artix7 FPGA are located under `public/A7_d2/.` 
Each target directory contains two sub-directories containing the
training set `target/vk0` and the validation set `target/fk0`. The value of the fields for each datasets are organised 
as file of \\( 2 ^ {24} \\) elements that are stored under subdirectories holding the name of the field.
```
public/
+-- A7_d2/
| +-- vk0/
...
data contained in a dataset per blocks of arbitrary size (see `dataset.py` or
